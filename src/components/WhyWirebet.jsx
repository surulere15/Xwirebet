    ...
    <section className="py-40 px-6 bg-background text-center flex flex-col items-center">
      <div className="max-w-6xl mx-auto w-full">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl font-display font-light tracking-wide text-white">Strategic Advantage</h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-10">
          {features.map((feature, idx) => (
            <motion.div 
              ...
              className="bg-white/[0.01] border border-white/10 p-12 hover:bg-white/[0.03] transition-colors duration-700 flex flex-col items-center text-center group"
            >
              <h3 className="text-lg md:text-xl font-display text-white mb-6 tracking-wide group-hover:text-zinc-200 transition-colors">{feature.title}</h3>
              <p className="text-zinc-400 font-light leading-relaxed tracking-wide text-sm md:text-base">{feature.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
